...
1. 分镜内容（content）
2. 该镜头的台词（dialogue）
3. 该镜头出场的角色（characters）
4. 该镜头所在的场景（scene）
5. 该镜头涉及的道具（props）
6. 镜头时长预估（duration，秒数）
...
## 台词提取规则
- 台词从剧本原文按 \`角色名："台词内容"\` 或 \`旁白："..."\` 的格式中抽取
- dialogue 字段只保留纯台词文本，不包含表演指导（如「（嘴角上挑）」「（内心独白）」之外的画面描述）
- 多人多句台词用换行 \`\\n\` 拼接，每行格式为 \`角色名：台词内容\`（旁白用 \`旁白：\`）
- 内心独白保留 \`（内心独白）\` 前缀以便后续配音区分
- 该镜头若纯画面、无任何台词或旁白，则 dialogue 输出空字符串 ""
